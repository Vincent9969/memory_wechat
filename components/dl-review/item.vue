	...
	<view class="content">
		<view class="cenHost">
			<view class="cenHeadImgContent">
				<image class="headImg" :src="reviewMsg.headurl"></image>
			</view>
			<view class="cenHostMsgContent">
				<view class="viewMb viewMb-space-between">
					<view>
						<text class="textSize">{{reviewMsg.plrname}}</text>
...
<!-- 					<view>
				...
				<view class="cenHostReview viewMb">
					<text class="textSendMsg">{{reviewMsg.plnr}}</text>
				</view>
				<view class="iconRow">
					<view>
						<text class="cenHostMsg4 textCenMsg">{{reviewMsg.plsj}}</text>
					</view>
					<view style="position: absolute;right: 20upx;" @click="reviewMsgFuc(reviewMsg.plrname,reviewMsg.plrxh,reviewMsg.xh)">
						<dnIcon type="xinxi" color="#C0C0C0"></dnIcon>
					</view>
				</view>
				<view class="threeReviewContent"  v-if="reviewMsg.reviewless.length">
					<view class="threeReviewVueText" v-for="(reKey, key) in reviewMsg.reviewless" :key="key" @click="reviewMsgFuc(reKey.hfrname,reKey.hfrxh,reKey.plxh)">
						{{reKey.hfrname}}
						<text class="defaultBlack">回复</text>
						{{reKey.plrname}}
						<text class="defaultBlack">:{{reKey.hfnr}}</text>
					</view>
					<view class="reviewNumContent" v-if="reviewMsg.reviewnum > 5" @click="openChildReview(reviewMsg)">
