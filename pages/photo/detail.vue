...
<!-- 				<view class="detail-content-view">
				<view class="detail-text">图片很好看，我很喜欢，我很喜欢，我很行</view>
				</view> -->
			</swiper-item>
		</swiper>
		<!-- #ifndef H5 -->
		<!-- <view class="detail-btn-view"></view> -->
		<view class="detail-content-view">
...
export default {
	...
	onLoad(e) {
		...
		this.imgLength = data.imgnum;
		var img={};
		img.imgurl=data.zpcfmimgurl;
		this.data.push(img);
		this.getData(data.xh);
	...
	methods: {
		...
		preImg(index) {
			...
			setTimeout(() => {
				uni.previewImage({
					current: this.data[index].imgurl,
					urls: this.getImgUrls(this.data)
				});
			}, 150);
		},
		getImgUrls(imgList) {
		...
		getData(zpzxh) {
			...
			uni.request({
				...
				url: this.websiteUrl + '/img/getImgListByGroupXh',
				method: 'POST',
				data: { zpzxh: zpzxh },
				success: res => {
					uni.hideLoading();
